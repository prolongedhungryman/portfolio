...
/* ═══════════════════════════════════════════
   ELEMENTS — safe, null-checked throughout
════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════
   CANVAS MASK
   Fixes applied:
   1. DPR-correct buffer with CSS pixel coords
   2. rAF-batched — one draw per frame max
      (both vibeProxy + darkProxy share one queue)
   3. destination-out MUST use opaque fill
      (white was semi-transparent = bad punch)
   4. Immediate _flushMask() on init, not deferred
════════════════════════════════════════════ */
let currentScale = 1;
let currentDark  = 1;
let maskRafId    = null;
...
function drawMask(scale, darkOpacity) {
    ...
    if (maskRafId) return;                          // already queued this frame
...
function _flushMask() {
    ...
    // 2. Punch VIBE hole — opaque black for correct destination-out
...
/* ═══════════════════════════════════════════
   ...
   Only one seek per animation frame.
   Threshold 0.1s avoids decoder queue overload
   on fast scrolls (was 0.04 = too tight).
════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════
   HOMEPAGE BG VIDEO
   Strategy for instant play on section enter:
   • Call .load() NOW at script boot — browser
     starts buffering in background immediately
   • IntersectionObserver plays/pauses cleanly
   • In CSS: position absolute (not fixed) inside
     #homepage so it scrolls away with the section
     instead of bleeding into other sections
   
   ⚠️  YOU MUST RUN THIS FFMPEG COMMAND ONCE
       on your video before deploying:
   
   ffmpeg -i assets/video/secondbg.mp4 \
     -vcodec libx264 -crf 23 -preset slow \
     -movflags faststart -vf "scale=1920:-2" \
     assets/video/secondbg.mp4
   
   -movflags faststart moves the moov atom to
   the FRONT of the file. Without it the browser
   must download the entire file before playing
   any frame — that's your 10-20s delay.
════════════════════════════════════════════ */
const homeBgVideo = document.getElementById("homepage-bg-video");
if (homeBgVideo) {
    homeBgVideo.load();   // start buffering immediately on page load
...
/* ═══════════════════════════════════════════
   LOADER GATE
   Waits for both fonts AND video metadata so
   canvas font sizing is correct on first draw.
════════════════════════════════════════════ */
let started = false;
...
function buildVibe() {
    ...
    currentScale = 1;
    currentDark  = 1;
    _flushMask();   // immediate synchronous first draw
    ...
    const tl = gsap.timeline({
        scrollTrigger: {
            ...
            scrub:   1,             // smooth lerp — was: true (instant/jerky)
            pin:     true,
            anticipatePin:       1,
    ...
    tl.fromTo(vibeProxy,
        ...
        { scale: 14, ease: "none", duration: 1,
          onUpdate: () => drawMask(vibeProxy.scale, darkProxy.opacity) }, 0
    );
    tl.fromTo("#lazy-text",
        { x: 0,    opacity: 0.85 },
        { x: -160, opacity: 0, ease: "none", duration: 0.4 }, 0
    );
    tl.fromTo("#ek-text",
        { x: 0,   opacity: 0.85 },
        { x: 160, opacity: 0, ease: "none", duration: 0.4 }, 0
    ...
    tl.fromTo(darkProxy,
        ...
        { opacity: 0, ease: "none", duration: 0.3,
          onUpdate: () => drawMask(vibeProxy.scale, darkProxy.opacity) }, 0.7
    );
...
function buildHomepage() {
    ...
    gsap.utils.toArray("#reveal-headline .word").forEach((word, i) => {
        gsap.to(word, {
            ...
            delay: i * 0.12,
            scrollTrigger: {
                trigger: "#reveal-headline", start: "top 75%",
...
/* ═══════════════════════════════════════════
   CURSOR REVEAL
   Consolidated here — remove cursor-reveal.js
   from your HTML to avoid duplicate conflicts.
════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════
   PIXEL SNOW
   Fixed: rafId properly tracked so animation
   is cancelled when section leaves viewport
   (was spawning runaway infinite rAF loops)
════════════════════════════════════════════ */
(function () {
    ...
    function spawn() {
        ...
        flakes.push({ x: Math.random() * (snowCanvas.width || 800), y: -r * 2, r,
